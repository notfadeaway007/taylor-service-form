...
exports.handler = async function(event) {
  ...
  try {
    ...
    const subject = `Taylor Grill Service Contract — ${business_name} — Model ${model_number}`;
    const pdfFilename = `GrillServiceContract_${business_name.replace(/[^a-zA-Z0-9]/g,'_')}_${model_number}.pdf`;
    ...
    const htmlBody = `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
        <div style="background:#102033;padding:20px 24px;">
          ...
          <p style="color:#29c3f0;margin:4px 0 0;font-size:12px;letter-spacing:3px;">GRILL EXTENDED SERVICE CONTRACT</p>
        </div>
        <div style="background:#fffde7;border-left:4px solid #deac5c;padding:10px 16px;">
          <p style="color:#6a5000;font-size:13px;font-weight:bold;margin:0;">
            ⚠ New grill service contract application received. PDF attached — please sign and submit to Taylor Company for factory approval.
          </p>
        </div>
        <div style="padding:20px 24px;background:#f0f6fb;">
          <table style="width:100%;border-collapse:collapse;font-size:13px;">
            ...
            <tr style="background:#fff;"><td style="padding:7px 10px;color:#5a7a92;font-size:11px;text-transform:uppercase;width:40%;">Name of Applicant</td><td style="padding:7px 10px;font-weight:bold;color:#102033;">${applicant_name}</td></tr>
            <tr style="background:#f7fbff;"><td style="padding:7px 10px;color:#5a7a92;font-size:11px;text-transform:uppercase;">Business Name</td><td style="padding:7px 10px;font-weight:bold;color:#102033;">${business_name}</td></tr>
            <tr style="background:#fff;"><td style="padding:7px 10px;color:#5a7a92;font-size:11px;text-transform:uppercase;">Machine Location</td><td style="padding:7px 10px;font-weight:bold;color:#102033;">${address}, ${city}, ${state} ${zip}</td></tr>
            ...
            <tr style="background:#fff;"><td style="padding:7px 10px;color:#5a7a92;font-size:11px;text-transform:uppercase;">Reply-To Email</td><td style="padding:7px 10px;font-weight:bold;color:#102033;">${email}</td></tr>
            <tr><td colspan="2" style="background:#102033;color:#29c3f0;font-weight:bold;padding:8px 10px;font-size:11px;text-transform:uppercase;letter-spacing:1px;">Grill</td></tr>
            <tr style="background:#fff;"><td style="padding:7px 10px;color:#5a7a92;font-size:11px;text-transform:uppercase;">Grill Model No.</td><td style="padding:7px 10px;font-weight:bold;color:#102033;">${model_number}</td></tr>
